...
| Folder Trust (first-time-per-folder consent, VS Code/Cursor-style) | 📝 Designed, not built | `~/.codeagent/trustedFolders.json` (realpath-keyed, global), `--trust` flag (distinct from `--yolo` — different question), `codeagent trust list/revoke`. See `docs/30`. |
...
| [30 — Folder Trust](./docs/30-folder-trust.md) | First-time-per-folder consent gate (design only, not yet implemented) |
