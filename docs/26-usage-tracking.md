...
- `khanagent usage` (default: this month) / `--today` / `--week` / `--all` — per-provider/model breakdown (calls, tokens, estimated cost) plus a total and the pricing date.
- `khanagent quota` — shows every configured quota's current status.
- `khanagent quota set <provider> <limitUSD>` — sets or updates one provider's monthly limit.
