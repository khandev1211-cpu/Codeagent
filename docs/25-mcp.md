...
2. **Transparency** — the name itself signals to the model, and to anyone reading a confirmation prompt or audit log, that a given call is going out to third-party code, not khanagent's own. This matters specifically because MCP tool results are untrusted third-party output — same caution `run_bash`'s stdout already gets — and a name that makes the origin obvious is a small but real part of that.
...
## `.khanagent/mcp.json` and its trust boundary
...
MCP servers are real subprocesses. `connectAllMcpServers` is called once per session (one-shot CLI invocation, REPL startup, or TUI startup) rather than per-turn, to avoid reconnect overhead on every message — and `closeAllMcpClients` runs in a `finally` block (one-shot) or at loop/app exit (REPL, TUI) specifically so a session ending doesn't leak server processes. `khanagent mcp` (the CLI visibility command, mirroring `khanagent skills`/`khanagent subagents`/`khanagent hooks`) connects, lists, and then also closes — it's a diagnostic snapshot, not a persistent connection.
...
- **Personal (`~/.khanagent/mcp.json`) or plugin-bundled server configs** — deferred to the Plugins phase, same as Skills, Subagents, and Slash Commands.
