...
This document is the contract for every tool the agent can call — its "skills." Each tool is a self-contained module in `src/tools/` exporting the same shape, so the Registry can auto-collect them and the orchestrator never needs tool-specific logic.
