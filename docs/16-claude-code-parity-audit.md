...
| **MCP client support** | Connect external tool servers (stdio/HTTP/SSE) — databases, issue trackers, browsers — as additional tools the model can call. | ✅ Shipped 2026-08-11 (see `docs/25`). Official `@modelcontextprotocol/sdk`, stdio transport (HTTP/SSE deferred, not silently dropped). `.codeagent/mcp.json`, `mcp__<server>__<tool>` naming, `destructive: true` unless a server's `readOnlyHint` annotation clears the bar. Verified against a real subprocess MCP server, not mocked (`test/mcp/realServer.integration.test.js`). `codeagent mcp` for visibility. **Not built:** personal/plugin-bundled server configs, MCP Resources/Prompts (only Tools), mid-session `listChanged`. | None — additive to the Tool Registry as predicted below, no changes needed to `orchestrator.js`'s dispatch logic at all. |
| **Plugins** | A *packaging and distribution* format — a plugin bundles skills + subagents + hooks + MCP servers + a manifest, installed from a marketplace (GitHub/git/npm/local path). | ❌ None yet. `docs/11` has a good blueprint but it's scoped only to *tools* (`codeagent-plugin-*` → Tool Registry), not the full skills+agents+hooks+MCP bundle real plugins are. **Now unblocked** — Skills, Subagents, Hooks, and MCP all exist (this was the one item deliberately waiting on all four). | This is genuinely the last item — a plugin bundling nothing (because Skills/Subagents/Hooks/MCP didn't exist yet) would have shipped an empty wrapper. |
...
| **Memory (`CLAUDE.md` equivalent)** | Auto-loaded, directory-layered project instructions (`CLAUDE.md`, `.claude/rules/`), separate from a single global config string. | ✅ Shipped 2026-08-11 (see `docs/23`). `AGENTS.md` at the project root + `~/.codeagent/AGENTS.md` for personal cross-project preferences, both auto-loaded every session, rendered between the admin prompt and the skills index. `codeagent memory` for visibility. **Not built**, deliberately: nested/directory-layered discovery (codeagent's session model has one `cwd`, not a natural per-directory hook point) and `@import` syntax — both real gaps relative to Claude Code, tracked as open, not silently dropped. | None — was additive to `context.js`/`systemPrompt.js`, as predicted. |
| **Slash commands** | Reusable custom prompts as files (`/review`, `/pr-description`), project- or user-scoped, with argument substitution. | ✅ Shipped 2026-08-11 (see `docs/24`). Built-ins (`/help`, `/clear`, `/plan`) plus project-scoped custom commands (`.codeagent/commands/<name>.md`, `$ARGUMENTS` substitution). Also closed the `/plan` in-REPL toggle PLAN.md had deferred pending this. `codeagent commands` for visibility. **Not built:** personal (`~/.codeagent/commands/`) or plugin-bundled commands (deferred to Plugins), namespacing/subdirectories, per-command tool restriction. | None — separate from CLI commands, lives in the REPL/TUI layer, as predicted. |
