...
describe("loadHooksConfig", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "khanagent-hooks-test-"));
  });
  ...
  it("returns an empty hooks object when .khanagent/hooks.json doesn't exist", () => {
  ...
  it("loads and validates a well-formed hooks.json", () => {
    ...
    fs.writeFileSync(
      path.join(tmpDir, ".khanagent", "hooks.json"),
      JSON.stringify({
        hooks: {
  ...
  it("throws ConfigError on malformed JSON", () => {
    fs.mkdirSync(path.join(tmpDir, ".khanagent"));
    fs.writeFileSync(path.join(tmpDir, ".khanagent", "hooks.json"), "{ not valid json");
