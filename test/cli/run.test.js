 ...
 * key — a corrupted `~/.khanagentrc` that happens to also be missing (or
...
describe("run() — corrupted config on what looks like first run", () => {
  ...
  it("does not throw uncaught when the config file is invalid and configExists() also reports false", async () => {
    homedir = fs.mkdtempSync(path.join(os.tmpdir(), "khanagent-run-"));
    // No `providers` key at all -> configExists() is false -> looks like
    // first run -> but the file DOES exist and IS invalid.
    fs.writeFileSync(path.join(homedir, ".khanagentrc"), JSON.stringify({ maxIterationsPerTurn: "not-a-number" }));
    ...
    try {
      await expect(run(["node", "khanagent", "config", "validate"])).resolves.not.toThrow();
    } finally {
